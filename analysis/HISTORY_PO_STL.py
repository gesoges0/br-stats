...
def export_history_TplDbl(stats_type):
    _from = 1947
    _to = 2021
    ...
    for year in range(_from, _to):
        ...
        # レギュラーシーズン
        # for record in session.query(TotalsRecordRegularSeason).filter(TotalsRecordRegularSeason._Season == season, TotalsRecordPlayOffs._Lg == 'NBA').all():
        #     if stats_type not in record.__dict__.keys():
        #         continue
        #     player_dict[record.id].stats_list[year - _from] = getattr(record, stats_type) if getattr(record,stats_type) else 0
        #     player_dict[record.id].team_list[year - _from] = getattr(record, '_Tm') if getattr(record, '_Tm') else ''
        
        # プレイオフ
        for record in session.query(TotalsRecordPlayOffs).filter(TotalsRecordPlayOffs._Season == season, TotalsRecordPlayOffs._Lg == 'NBA').all():
            if stats_type not in record.__dict__.keys():
                continue
            player_dict[record.id].stats_list[year - _from] += getattr(record, stats_type) if getattr(record,stats_type) else 0
            # player_dict[record.id].team_list[year - _from] = getattr(record, '_Tm') if getattr(record, '_Tm') else ''
    ...
    # 結果を出力
    output_file = './output/HISTORY_PO_STL_new.csv'
    session = create_session('NBA3')
...
if __name__ == '__main__':
    export_history_TplDbl(stats_type='_STL')
