...
def export_2020_21_regular_season_3P():
    ...
    for gamelog_record in session.query(RegularSeasonRecord).filter(_from <= RegularSeasonRecord._Date, RegularSeasonRecord._Date <= _to).all():
        ...
        # gamelogをplayerに追加
        if gamelog_record._3P is None:
            tp = 0
        else:
            tp = gamelog_record._3P
        players_dict[gamelog_record.id].stats_list[dayindex(_from, gamelog_record._Date)] = tp
    ...
    # csv へ出力
    output_file_path = './output/2020_21_regular_season_3P.csv'
...
if __name__ == '__main__':
    export_2020_21_regular_season_3P()
